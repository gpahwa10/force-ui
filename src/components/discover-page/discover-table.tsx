...
// Generate positions from teams
const generatePositionsFromTeams = (teams: Team[]): Position[] => {
  return teams.map((team) => {
    // Mock team price data (similar to athlete prices)
    ...
    const pnl = currentPrice - entryPrice;
    const pnlPercentage = (pnl / entryPrice) * 100;
    const indexPercent = 70 + Math.random() * 30; // Random index between 70-100

    return {
      id: team.id,
      teamName: team.name,
      teamImage: team.logoUrl,
      team: team.name,
      indexPercent: Number(indexPercent.toFixed(2)),
      position: Math.random() > 0.5 ? "Long" : "Short",
      leverage: `${Math.floor(Math.random() * 5) + 1}x`,
      size: Math.floor(Math.random() * 1000) + 500, // Teams typically have larger sizes
      entryPrice: Number(entryPrice.toFixed(2)),
      currentPrice: Number(currentPrice.toFixed(2)),
      pnl: Number(pnl.toFixed(2)),
      pnlPercentage: Number(pnlPercentage.toFixed(2)),
...
// Get initial positions from athletes and teams
const initialAthletes = getRandomAthletes(5, false);
const initialTeams = getAllTeams();
const defaultAthletePositions = generatePositionsFromAthletes(initialAthletes);
const defaultTeamPositions = generatePositionsFromTeams(initialTeams);
const defaultPositions = [...defaultAthletePositions, ...defaultTeamPositions];
...
export default function DiscoverTable({
  ...
  const [entityType, setEntityType] = useState<"athlete" | "team">("athlete");
  ...
  // Filter positions based on entity type and search query
  const filteredPositions = useMemo(() => {
    // First filter by entity type (athlete or team)
    let filtered = positions.filter((position) => position.type === entityType);
    ...
    if (searchQuery.trim()) {
      ...
      filtered = filtered.filter(
        (position) =>
          ...
          (position.teamName?.toLowerCase().includes(query)) ||
          position.team.toLowerCase().includes(query),
      );
    }

    return filtered;
  }, [positions, entityType, searchQuery]);
  ...
  return (
    <div className="bg-elevation-card flex flex-col items-start gap-5 self-stretch overflow-hidden rounded-[20px] px-3 pt-1 pb-3 md:px-5">
      ...
      <div className="flex flex-wrap items-center justify-center gap-4 self-stretch border-b">
        {/* Entity Type Tabs (Athlete/Team) */}
        <div className="flex items-center gap-0">
          <button
            onClick={() => setEntityType("athlete")}
            className={`flex w-[100px] items-center justify-center gap-2.5 border-b-2 px-0 py-5 ${entityType === "athlete"
              ? "border-border-secondary"
              : "border-elevation-bg"
              }`}
          >
            <span
              className={`text-[13px] leading-[13px] font-medium tracking-[-0.1px] ${entityType === "athlete"
                ? "text-text-primary"
                : "text-text-secondary"
                }`}
            >
          ...
          <button
            ...
            className={`flex w-[100px] items-center justify-center gap-2.5 border-b-2 px-0 py-5 ${entityType === "team"
              ? "border-border-secondary"
              : "border-elevation-bg"
              }`}
          >
            <span
              className={`text-[13px] leading-[13px] font-medium tracking-[-0.1px] ${entityType === "team"
                ? "text-text-primary"
                : "text-text-secondary"
                }`}
            >
            ...
            <TableHeader>
              <TableRow className="border-0 bg-transparent">
                <TableHead className="text-text-secondary w-[260px] border-0 px-3 py-2.5 text-[11px] leading-[11px] font-medium tracking-[-0.1px]">
                  {entityType === "athlete" ? "Athlete / Index" : "Team / Index"}
                </TableHead>
                    ...
                    <div className="flex items-center gap-3 pr-[30px]">
                      <div className="flex flex-1 items-center gap-2">
                        <div className="relative aspect-square h-8 w-8 overflow-hidden rounded-[29.463px] bg-gray-200">
                          <Image
                            src={position.type === "athlete" ? position.athleteImage! : position.teamImage!}
                            alt={position.type === "athlete" ? position.athleteName! : position.teamName!}
                            fill
                        ...
                        <div className="flex flex-col items-start justify-center gap-1">
                          <span className="text-text-primary text-[13px] leading-[13px] font-semibold tracking-[-0.1px]">
                            {position.type === "athlete" ? position.athleteName : position.teamName}
                          </span>
                          {position.type === "athlete" && (
